...
let SurfaceDetection = class SurfaceDetection extends BaseScriptComponent {
    ...
    onCalibrationComplete() {
        ...
        // **ADD THIS - Actually set the world position of the object with SyncTransform**
        if (this.visualObj) {
            this.visualObj.getTransform().setWorldPosition(this.calibrationPosition);
            this.visualObj.getTransform().setWorldRotation(this.calibrationRotation);
            print("SurfaceDetection: Applied position to visualObj: " + this.calibrationPosition);
        }
        ...
        // Add debug logging
        print("SurfaceDetection: Calibration complete!");
        // Notify the Game Manager that surface tracking is complete
        if (global.onSurfaceTrackingPlaced) {
            print("SurfaceDetection: Calling global.onSurfaceTrackingPlaced");
            global.onSurfaceTrackingPlaced();
        }
        else {
            print("SurfaceDetection: ERROR - global.onSurfaceTrackingPlaced not found!");
        }
    }
    __initialize() {
        super.__initialize();
