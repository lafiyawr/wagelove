...
let FireflyController = class FireflyController extends BaseScriptComponent {
    onAwake() {
        ...
        const rotateFF1 = LSTween_1.LSTween.rotateOffset(ff1Transform, quat.angleAxis(MathUtils.DegToRad * 360, vec3.up()), 20000, RotationInterpolationType_1.RotationInterpolationType.LERP).repeat(Infinity);
        const rotateFF2 = LSTween_1.LSTween.rotateOffset(ff2Transform, quat.angleAxis(MathUtils.DegToRad * 360, vec3.up()), 25000, RotationInterpolationType_1.RotationInterpolationType.LERP).repeat(Infinity);
        const rotateFF3 = LSTween_1.LSTween.rotateOffset(ff3Transform, quat.angleAxis(MathUtils.DegToRad * 360, vec3.up()), 30000, RotationInterpolationType_1.RotationInterpolationType.LERP).repeat(Infinity);
        rotateFF1.start();
        rotateFF2.start();
        rotateFF3.start();
        // Glow fade animation (0.2 → 1.0 → 0.2)
        const alphaTween = LSTween_1.LSTween.alphaFromTo(material, 0.2, 1.0, 20000)
            .easing(Easing_1.default.Circular.InOut)
            .yoyo(true)
            .repeat(Infinity);
        alphaTween.start();
