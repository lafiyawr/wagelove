...
export class StartMenu extends BaseScriptComponent {
    ...
    private onSinglePlayerPress() {
        switch (this.singlePlayerType) {
            ...
            default:
                ...
                if ((global as any).startSinglePlayerWrapper) {
                    (global as any).startSinglePlayerWrapper()
                } else {
    ...
    private startMultiplayerSession() {
        ...
        delayedEvent.bind(() => {
            if ((global as any).startMultiplayerWrapper) {
                (global as any).startMultiplayerWrapper()
            } else {
                this.log.w("Game Manager not ready - falling back to original behavior")
            }
        })
