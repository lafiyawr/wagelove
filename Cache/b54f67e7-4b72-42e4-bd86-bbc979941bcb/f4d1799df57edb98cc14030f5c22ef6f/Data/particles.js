...
function handleEmission() {
    ...
    if (script.burstCount > 0) {
        // Burst mode
        if (currentTime - lastSpawnTime > 1.0 / script.spawnRate) {
            spawnBurst(script.burstCount);
            lastSpawnTime = currentTime;
        }
    } else {
        // Continuous mode
        var spawnInterval = 1.0 / script.spawnRate;
...
function updateParticlePhysics(particle, age) {
    // Apply forces
    particle.velocity = particle.velocity.add(script.gravity.scale(getDeltaTime()));
    particle.velocity = particle.velocity.add(script.wind.scale(getDeltaTime()));
    
    // Apply drag
    particle.velocity = particle.velocity.scale(script.drag);
