...
function onStart() {
    ...
    // Initialize internal variables with script input values (READ from inputs, don't assign TO them)
    currentSpawnRate = script.spawnRate;
    currentGravity = new vec3(script.gravity.x, script.gravity.y, script.gravity.z);
    currentWind = new vec3(script.wind.x, script.wind.y, script.wind.z);
...
function destroyParticle(particleData) {
    ...
    // Try to fade out if possible (this might not work with all prefabs)
    try {
        ...
        // Ignore fade errors for prefabs without compatible renderers
    }
...
function setGravity(gravity) {
    currentGravity = new vec3(gravity.x, gravity.y, gravity.z);
    print("Gravity set to: " + gravity.toString());
}

function setWind(wind) {
    currentWind = new vec3(wind.x, wind.y, wind.z);
