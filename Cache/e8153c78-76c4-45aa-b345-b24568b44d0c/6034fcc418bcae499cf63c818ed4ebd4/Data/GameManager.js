...
function playIntro() {
    ...
    delayedEvent2.bind(function() {
        // HOST/SINGLE PLAYER controls transition to ancestor intro
        var isSinglePlayerMode = !global.sessionController || totalPlayersCount <= 1;
        if ((syncEntity && syncEntity.doIOwnStore()) || isSinglePlayerMode) {
            print("HOST/SINGLE PLAYER: Sending transition to ancestor intro");
            sendShowFireflies();
        }
    });
...
function playAncestorIntro() {
    ...
    // HOST controls transition to ancestor selection (works in single player too)
    var delayedEvent = script.createEvent("DelayedCallbackEvent");
    delayedEvent.bind(function() {
        var isSinglePlayerMode = !global.sessionController || totalPlayersCount <= 1;
        if ((syncEntity && syncEntity.doIOwnStore()) || isSinglePlayerMode) {
            print("HOST/SINGLE PLAYER: Sending transition to ancestor selection");
            sendAncestorsRise();
        }
    });
