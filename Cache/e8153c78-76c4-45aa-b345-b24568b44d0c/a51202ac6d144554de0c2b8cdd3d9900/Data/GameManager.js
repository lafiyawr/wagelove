...
function showObjectsPlaced() {
    ...
    if (isHost()) {
        ...
        print("Non-host: Enabling surface detection object for content visibility");
        // For non-host: enable the object but keep scripts disabled
        if (script.surfaceDetection) {
            script.surfaceDetection.enabled = true;
        }
        ...
        updateHostWaitingMessage();
        enableReadyButton(false);
        // Keep surface tracking scripts disabled for non-host
...
function sendObjectsPlaced() {
    if (syncEntity && syncEntity.doIOwnStore()) {
        ...
        // Get the surface detection transform data to sync
        var transformData = null;
        if (script.surfaceDetection) {
            var transform = script.surfaceDetection.getTransform();
            ...
            print("Sending transform data: position(" + worldPos.x + "," + worldPos.y + "," + worldPos.z + ")");
            print("Sending transform data: rotation(" + worldRot.x + "," + worldRot.y + "," + worldRot.z + "," + worldRot.w + ")");
        }
        ...
        print("Objects placed event sent to all players");
    } else {
...
function onReceiveObjectsPlaced(eventData, messageInfo) {
    ...
    // Apply the surface tracking transform if we're not the host and have transform data
    if (!isHost() && eventData && eventData.transform && script.surfaceDetection) {
        var transform = script.surfaceDetection.getTransform();
        
        // Convert the received data back to vec3 and quat
        var newPosition = new vec3(
            eventData.transform.position.x,
            eventData.transform.position.y,
            eventData.transform.position.z
        );
        var newRotation = new quat(
            eventData.transform.rotation.w,
            eventData.transform.rotation.x,
            eventData.transform.rotation.y,
            eventData.transform.rotation.z
        );
        
        transform.setWorldPosition(newPosition);
        transform.setWorldRotation(newRotation);
        
        print("Applied transform to non-host player: position(" + newPosition.x + "," + newPosition.y + "," + newPosition.z + ")");
        
        // Enable the surface detection object for content visibility
        script.surfaceDetection.enabled = true;
        print("Surface detection object enabled for content visibility");
    } else if (isHost()) {
        print("Host received own objects placed event - ignoring transform sync");
    } else {
        print("WARNING: No transform data received or missing surface detection object");
    }
