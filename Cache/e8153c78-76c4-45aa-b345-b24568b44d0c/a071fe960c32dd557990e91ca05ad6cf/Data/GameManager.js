...
function showReadyToStart() {
    ...
    if (isHost()) {
        // This is the host - show ready button and surface tracking
        print("You are the host - showing ready button");
        setUIVisibility("waitingMessage", false);
        setUIVisibility("readyButton", true);
        setUIVisibility("hostWaitingMessage", false);
        
        // Enable surface tracking for host only
        ...
        // This is not the host - show waiting message
        print("You are not the host - showing waiting message");
        setUIVisibility("waitingMessage", false);
        setUIVisibility("readyButton", false);
        setUIVisibility("hostWaitingMessage", true);
        
        // Disable surface tracking for non-hosts
        enableSurfaceTracking(false);
...
function enableSurfaceTracking(enable) {
    // Enable/disable device tracking component
    if (script.deviceTrackingComponent) {
        script.deviceTrackingComponent.enabled = enable;
    }
    
    // Enable/disable surface tracking UI
