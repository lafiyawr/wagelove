...
function playIntro() {
    ...
    delayedEvent2.bind(function() {
        // HOST controls transition to ancestor intro
        if (syncEntity && syncEntity.doIOwnStore()) {
            print("HOST: Sending transition to ancestor intro");
            sendShowFireflies(); // Reuse this event for ancestor intro transition
        }
    });
    delayedEvent2.reset(8.0);
}
...
function playAncestorIntro() {
    ...
    delayedFireflies.bind(function() {
        ...
        // Add your fireflies logic here
        script.gameObjects[1].enabled = true;
    });
    delayedFireflies.reset(6.0);
    
    // HOST controls transition to ancestor selection
    var delayedEvent = script.createEvent("DelayedCallbackEvent");
    delayedEvent.bind(function() {
        if (syncEntity && syncEntity.doIOwnStore()) {
            print("HOST: Sending transition to ancestor selection");
            sendAncestorsRise();
        }
    });
    delayedEvent.reset(12.0);
}
...
function onReceiveShowFireflies(eventData, messageInfo) {
    print("Received show fireflies event - transitioning to ancestor intro");
