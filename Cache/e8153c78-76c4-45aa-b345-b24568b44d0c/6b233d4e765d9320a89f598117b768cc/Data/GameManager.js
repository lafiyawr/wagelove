...
function init() {
    ...
    currentState = GameState.MENU; // Set state but don't trigger state change logic
}
...
function showWaitingForPlayers() {
    ...
    // Check immediately if SyncEntity is ready, otherwise wait for onSyncReady to call checkHostStatusAndShowUI
    if (syncEntity && isHost() !== undefined) {
        // SyncEntity is ready, check immediately
        checkHostStatusAndShowUI();
    } else {
        // SyncEntity not ready yet - onSyncReady will call checkHostStatusAndShowUI when ready
        print("SyncEntity not ready - will check host status when ready");
        
        // Show default non-host UI for now
...
function checkHostStatusAndShowUI() {
    ...
    if (isHost()) {
        // Host sees waiting message and surface tracking immediately
        print("You are the host - showing surface tracking");
        setUIVisibility("waitingMessage", true);
        setUIVisibility("playerCountText", true);
        setUIVisibility("hostWaitingMessage", false);
        enableReadyButton(false);
        
        // Enable surface tracking for host right away
        ...
        // Move to surface tracking state
        setState(GameState.SURFACE_TRACKING);
    } else {
        // Non-host sees waiting for host message
        print("You are not the host - waiting for host");
        setUIVisibility("waitingMessage", false);
        enableReadyButton(false);
        setUIVisibility("hostWaitingMessage", true);
        setUIVisibility("playerCountText", true);
...
function enableSurfaceTracking(enable) {
    // Enable/disable the surface tracking scene object
