...
function setupGlobalFunctions() {
    ...
    // Expose state and player count for debugging
    global.currentState = function() { return currentState; };
    global.totalPlayersCount = function() { return totalPlayersCount; };
...
function sendSyncEvent(eventName, eventData, localCallback) {
    ...
    if ((syncEntity && syncEntity.doIOwnStore()) || isSinglePlayerMode) {
        ...
        if (isSinglePlayerMode) {
            ...
            // In single player, call the local callback directly
            if (localCallback) {
...
// Network message senders using helper function
function sendStartIntro() {
    sendSyncEvent('startIntro', null, onReceiveStartIntro);
}

function sendShowFireflies() {
    sendSyncEvent('showFireflies', null, onReceiveShowFireflies);
}

function sendAncestorsRise() {
    sendSyncEvent('ancestorsRisemsg', null, onReceiveAncestorsRise);
}

function sendReadyToFuse() {
    sendSyncEvent('readytofusemsg', null, onReceiveReadyToFuse);
}
