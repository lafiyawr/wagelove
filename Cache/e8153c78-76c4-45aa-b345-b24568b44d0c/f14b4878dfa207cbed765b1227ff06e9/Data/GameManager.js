...
function sendAncestorsRise() {
    ...
    if (isSinglePlayerMode) {
        ...
        var isHost = syncEntity.doIOwnStore();
        
        if (isHost) {
            print("*** MULTIPLAYER HOST: Sending ancestors rise event immediately ***");
            ancestorsRiseTriggered = true;
            syncEntity.sendEvent('ancestorsRisemsg');
        } else {
            print("*** MULTIPLAYER NON-HOST: Trying to trigger, but giving host priority ***");
            
            // Give host a 3-second window to trigger first
            if (!ancestorsRiseTimeout) {
                print("*** Starting 3-second host priority timer ***");
                ancestorsRiseTimeout = script.createEvent("DelayedCallbackEvent");
                ancestorsRiseTimeout.bind(function() {
                    if (!ancestorsRiseTriggered) {
                        print("*** Host didn't trigger in time - NON-HOST taking over! ***");
                        ancestorsRiseTriggered = true;
                        syncEntity.sendEvent('ancestorsRisemsg');
                    }
                });
                ancestorsRiseTimeout.reset(3.0); // 3 second timeout
            }
        }
    }
