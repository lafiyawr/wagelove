...
function onReceiveObjectsPlaced(eventData, messageInfo) {
    ...
    if (!isCurrentHost) {
        print("Non-host player: Objects should now be visible due to SyncTransform");
        
        // Just ensure surface detection is enabled - SyncTransform should handle positioning
        if (script.surfaceDetection) {
            script.surfaceDetection.enabled = true;
            print("Surface detection object enabled");
        }
        
        // SyncTransform components should automatically sync the positions
        print("Relying on SyncTransform components for positioning");
        
        // Wait a moment to see if SyncTransform updates positions
        var delayedCheck = script.createEvent("DelayedCallbackEvent");
        delayedCheck.bind(function() {
            print("=== CHECKING IF SYNCTRANSFORM WORKED ===");
            if (script.surfaceDetection) {
                var pos = script.surfaceDetection.getTransform().getWorldPosition();
                print("Surface detection position: (" + pos.x + "," + pos.y + "," + pos.z + ")");
                
                if (pos.x === 0 && pos.y === 0 && pos.z === 0) {
                    print("WARNING: SyncTransform is not working - position still (0,0,0)");
                } else {
                    print("SUCCESS: SyncTransform worked - object positioned correctly");
                }
            }
        });
        delayedCheck.reset(2.0);
