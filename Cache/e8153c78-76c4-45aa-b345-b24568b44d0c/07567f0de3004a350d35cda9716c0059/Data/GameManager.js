...
function sendAncestorFusion() {
 print("*** sendAncestorFusion called ***");
    sendSyncEvent('ancestorFusion', null, onReceiveAncestorFusion);
    print("*** sendAncestorFusion completed ***");
}

function sendSyncEvent(eventName, eventData, localCallback) {
    var isSinglePlayerMode = !global.sessionController || totalPlayersCount <= 1;
    
    print("*** sendSyncEvent called for: " + eventName + " ***");
    print("*** isSinglePlayerMode: " + isSinglePlayerMode + " ***");
    print("*** isHost: " + (syncEntity && syncEntity.doIOwnStore()) + " ***");
    
    if ((syncEntity && syncEntity.doIOwnStore()) || isSinglePlayerMode) {
        print("*** Sending " + eventName + " event ***");
        
        if (isSinglePlayerMode) {
            print("*** SINGLE PLAYER: Calling " + eventName + " directly ***");
            if (localCallback) {
                localCallback(eventData);
            }
        } else {
            print("*** MULTIPLAYER: Sending " + eventName + " network event ***");
            if (syncEntity) {
                syncEntity.sendEvent(eventName, eventData);
                print("*** " + eventName + " network event sent ***");
            } else {
                print("*** ERROR: syncEntity is null ***");
            }
        }
    } else {
        print("*** Not host - not sending " + eventName + " ***");
    }
}
