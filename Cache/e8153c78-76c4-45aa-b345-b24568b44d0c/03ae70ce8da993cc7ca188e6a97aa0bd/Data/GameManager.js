...
function onReceiveObjectsPlaced(eventData, messageInfo) {
    ...
    if (!isCurrentHost) {
        print("Non-host player: Enabling objects and trusting SyncTransform to handle positioning");
        
        // Just enable the surface detection object and its children
        // The SyncTransform component should automatically sync the position
        // set by the host's surface detection
        if (script.surfaceDetection) {
            script.surfaceDetection.enabled = true;
            print("Surface detection object enabled");
        ...
        // Give SyncTransform time to work, then verify
        var delayedCheck = script.createEvent("DelayedCallbackEvent");
        delayedCheck.bind(function() {
            print("=== VERIFYING SYNCTRANSFORM POSITIONING ===");
            if (script.surfaceDetection) {
                var pos = script.surfaceDetection.getTransform().getWorldPosition();
                print("Surface detection position after sync: (" + pos.x + "," + pos.y + "," + pos.z + ")");
                
                if (pos.x === 0 && pos.y === 0 && pos.z === 0) {
                    print("WARNING: SyncTransform may not be configured correctly - still at (0,0,0)");
                    print("Check that the SyncTransform component is on the main object that contains the surface detection");
                } else {
                    print("SUCCESS: SyncTransform worked - content should be visible!");
                }
            }
        });
