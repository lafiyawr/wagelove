...
function showWaitingForPlayers() {
    ...
    if (syncEntity && isHost() !== undefined) {
        ...
        // IMPORTANT: Disable surface detection immediately for safety
        // Even before we know host status, disable surface detection as a precaution
        if (script.surfaceDetection) {
            script.surfaceDetection.enabled = false;
            print("Precautionary surface detection disable while waiting for sync");
        }
...
function checkHostStatusAndShowUI() {
    ...
    if (isHost()) {
        ...
        print("NON-HOST (Player 2+): Showing waiting message, COMPLETELY DISABLING surface detection");
        setUIVisibility("waitingMessage", false);
        enableReadyButton(false);
        setUIVisibility("hostWaitingMessage", true);
        setUIVisibility("playerCountText", true);
        ...
        // This must happen immediately when we detect non-host status
        disableSurfaceDetectionCompletely();
...
// NEW: Function to completely disable surface detection for non-host
function disableSurfaceDetectionCompletely() {
    print("DISABLING ALL SURFACE DETECTION for non-host player");
    
    if (script.surfaceDetection) {
        // First, disable the entire object
        script.surfaceDetection.enabled = false;
        print("Surface detection object DISABLED");
        
        // Then disable all script components as extra safety
        var scriptComponents = script.surfaceDetection.getComponents("Component.ScriptComponent");
        for (var i = 0; i < scriptComponents.length; i++) {
            if (scriptComponents[i]) {
                scriptComponents[i].enabled = false;
                print("Disabled surface detection script component " + i);
            }
        }
        
        // Disable device tracking component
        var deviceTrackingComponent = script.surfaceDetection.getComponent("Component.DeviceTracking");
        if (deviceTrackingComponent) {
            deviceTrackingComponent.enabled = false;
            print("Disabled DeviceTracking component");
        }
...
function showSurfaceTracking() {
    ...
    if (isHost()) {
        ...
        print("NON-HOST: Just showing waiting message, ENSURING surface tracking is disabled");
        setUIVisibility("waitingMessage", false);
        setUIVisibility("hostWaitingMessage", true);
        updateHostWaitingMessage();
        
        // Double-check that surface detection is disabled for non-host
        disableSurfaceDetectionCompletely();
    }
}
