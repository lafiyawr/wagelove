...
function onReceiveObjectsPlaced(eventData, messageInfo) {
    ...
    if (!isCurrentHost) {
        print("Non-host player: Enabling objects and checking VisualAnchor specifically");
        ...
        if (script.surfaceDetection) {
            ...
            // Find and focus on VisualAnchor specifically
            ...
            for (var i = 0; i < childCount; i++) {
                ...
                child.enabled = true;
                print("Enabled child: " + child.name);
                
                // Look for VisualAnchor in grandchildren
                ...
                for (var j = 0; j < grandChildCount; j++) {
                    ...
                    if (grandChild.name === "VisualAnchor") {
                        visualAnchor = grandChild;
                        print("Found VisualAnchor - enabling and checking position");
                        var anchorPos = grandChild.getTransform().getWorldPosition();
                        print("VisualAnchor position: (" + anchorPos.x + "," + anchorPos.y + "," + anchorPos.z + ")");
                        
                        // Enable all children of VisualAnchor
                        var anchorChildCount = grandChild.getChildrenCount();
                        for (var k = 0; k < anchorChildCount; k++) {
                            var anchorChild = grandChild.getChild(k);
                            anchorChild.enabled = true;
                            print("Enabled VisualAnchor child: " + anchorChild.name);
                        }
                    } else {
                        print("Enabled grandchild: " + grandChild.name);
                    }
                }
            }
            
            if (!visualAnchor) {
                print("ERROR: Could not find VisualAnchor object!");
            }
        }
        
        // Check VisualAnchor position after SyncTransform has time to work
        var delayedCheck = script.createEvent("DelayedCallbackEvent");
        delayedCheck.bind(function() {
            print("=== CHECKING VISUALANCHOR SYNC AFTER 3 SECONDS ===");
            if (script.surfaceDetection) {
                // Find VisualAnchor again and check its position
                var childCount = script.surfaceDetection.getChildrenCount();
                for (var i = 0; i < childCount; i++) {
                    var child = script.surfaceDetection.getChild(i);
                    var grandChildCount = child.getChildrenCount();
                    for (var j = 0; j < grandChildCount; j++) {
                        var grandChild = child.getChild(j);
                        if (grandChild.name === "VisualAnchor") {
                            var pos = grandChild.getTransform().getWorldPosition();
                            print("VisualAnchor position after sync: (" + pos.x + "," + pos.y + "," + pos.z + ")");
                            
                            if (pos.x !== 0 || pos.y !== 0 || pos.z !== 0) {
                                print("SUCCESS: VisualAnchor has correct position!");
                                
                                // Debug 3D model visibility issues
                                print("=== DEBUGGING 3D MODEL VISIBILITY ===");
                                
                                // Check all objects under VisualAnchor
                                var anchorChildCount = grandChild.getChildrenCount();
                                for (var k = 0; k < anchorChildCount; k++) {
                                    var anchorChild = grandChild.getChild(k);
                                    print("VisualAnchor child: " + anchorChild.name + " enabled: " + anchorChild.enabled);
                                    
                                    // Check if this child has mesh components (3D models)
                                    var meshRenderer = anchorChild.getComponent("Component.MeshVisual");
                                    if (meshRenderer) {
                                        print("  Found MeshVisual on " + anchorChild.name);
                                        print("  MeshVisual enabled: " + meshRenderer.enabled);
                                        
                                        // Check mesh and material safely
                                        try {
                                            var mesh = meshRenderer.getMesh();
                                            if (mesh) {
                                                print("  Has mesh: true");
                                            } else {
                                                print("  WARNING: No mesh assigned!");
                                            }
                                        } catch (e) {
                                            print("  Could not check mesh: " + e);
                                        }
                                        
                                        try {
                                            var material = meshRenderer.getMaterial(0);
                                            if (material) {
                                                print("  Has material: true");
                                            } else {
                                                print("  WARNING: No material assigned!");
                                            }
                                        } catch (e) {
                                            print("  Could not check material: " + e);
                                        }
                                        
                                        // Check object scale
                                        var scale = anchorChild.getTransform().getLocalScale();
                                        print("  Object scale: (" + scale.x + "," + scale.y + "," + scale.z + ")");
                                        if (scale.x < 0.01 || scale.y < 0.01 || scale.z < 0.01) {
                                            print("  WARNING: Object may be too small to see!");
                                        }
                                        
                                        // Try to make it more visible
                                        print("  MAKING OBJECT MORE VISIBLE:");
                                        anchorChild.getTransform().setLocalScale(new vec3(10, 10, 10));
                                        print("  Scaled up 10x for visibility test");
                                    }
                                    
                                    // Check nested objects too
                                    var nestedCount = anchorChild.getChildrenCount();
                                    for (var l = 0; l < nestedCount; l++) {
                                        var nested = anchorChild.getChild(l);
                                        print("  Nested object: " + nested.name + " enabled: " + nested.enabled);
                                        var nestedMesh = nested.getComponent("Component.MeshVisual");
                                        if (nestedMesh) {
                                            print("    Found MeshVisual on nested " + nested.name);
                                            print("    MeshVisual enabled: " + nestedMesh.enabled);
                                        }
                                    }
                                }
                                
                                // Check distance from camera
                                var camera = script.camObj || global.scene.getRootObject().getChild(0); // Fallback to first object
                                if (camera) {
                                    var camPos = camera.getTransform().getWorldPosition();
                                    var distance = pos.distance(camPos);
                                    print("Distance from camera to content: " + distance + " units");
                                    if (distance > 1000) {
                                        print("WARNING: Content is very far from camera (" + distance + " units)");
                                        print("Content might be outside visible range or appear very small");
                                    }
                                }
                                
                                print("DEBUGGING TIPS:");
                                print("1. Try looking around - content is at: (" + pos.x + "," + pos.y + "," + pos.z + ")");
                                print("2. Check if 3D models have materials assigned");
                                print("3. Verify lighting is adequate for the 3D models");
                                print("4. Check model scale - they might be too small/large");
                                
                            } else {
                                print("PROBLEM: VisualAnchor still at (0,0,0) - SyncTransform not working");
                                print("Check SyncTransform component settings on VisualAnchor");
                            }
                            break;
                        }
                    }
