...
function init() {
    ...
    // Immediately disable surface detection for all players by default
    // We'll re-enable it only for the host once sync is ready
    if (script.surfaceDetection) {
        script.surfaceDetection.enabled = false;
        print("Surface detection disabled on startup - will enable for host when sync ready");
    }
...
function checkHostStatusAndShowUI() {
    ...
    if (isHost()) {
        ...
        enableSurfaceTracking(true); // Enable surface tracking scripts for host
        ...
        // Keep surface detection completely disabled for non-host
        if (script.surfaceDetection) {
            script.surfaceDetection.enabled = false;
        }
        enableSurfaceTracking(false); // Disable surface tracking scripts for non-host
        updateHostWaitingMessage();
    }
...
function onSurfaceTrackingPlaced() {
    print("Surface tracking placed - checking if host");
    
    if (isHost()) {
        print("Host confirmed - sending objects placed event");
        sendObjectsPlaced();
    } else {
        print("ERROR: Non-host triggered surface tracking! This should not happen.");
        // Don't send the event if non-host somehow triggered this
    }
}
...
function sendObjectsPlaced() {
    if (syncEntity && syncEntity.doIOwnStore()) {
        print("Sending objects placed event (using SyncTransform components for positioning)");
        
        // Since you're using SyncTransform components, we don't need to manually send transform data
        // The SyncTransform components will handle position/rotation synchronization automatically
        var eventData = {
            timestamp: Date.now(),
        ...
        try {
            ...
            print("Objects placed event sent to all players - SyncTransform will handle positioning");
            
            // Also try sending a delayed version in case the first one doesn't go through
            var delayedEvent = script.createEvent("DelayedCallbackEvent");
            delayedEvent.bind(function() {
                if (syncEntity && syncEntity.doIOwnStore()) {
                    print("Sending delayed objects placed event as backup");
                    syncEntity.sendEvent('objectsPlaced', eventData);
                }
            });
...
function onReceiveObjectsPlaced(eventData, messageInfo) {
    print("*** PLAYER 2 RECEIVED OBJECTS PLACED EVENT ***");
    print("Received objects placed event from: " + (messageInfo ? messageInfo.userId : "unknown"));
    print("Event data: " + JSON.stringify(eventData));
    print("Current isHost() value: " + isHost());
    print("Current syncEntity.doIOwnStore() value: " + (syncEntity ? syncEntity.doIOwnStore() : "null"));
    
    var isCurrentHost = isHost();
    if (!isCurrentHost) {
        print("Non-host player: Enabling surface detection object for content visibility");
        
        // Enable the surface detection object so SyncTransform components can work
        if (script.surfaceDetection) {
            script.surfaceDetection.enabled = true;
            print("Surface detection object enabled - SyncTransform components will handle positioning");
            
            // Log current position before enabling children
            var currentPos = script.surfaceDetection.getTransform().getWorldPosition();
            print("Surface detection current position: (" + currentPos.x + "," + currentPos.y + "," + currentPos.z + ")");
            
            // Also enable all child objects to make sure content is visible
            var childCount = script.surfaceDetection.getChildrenCount();
            print("Found " + childCount + " child objects to enable");
            for (var i = 0; i < childCount; i++) {
                ...
                print("Enabled child object: " + child.name + " at position (" + childPos.x + "," + childPos.y + "," + childPos.z + ")");
            }
            
            // Wait a moment and check positions again to see if SyncTransform updated them
            var delayedCheck = script.createEvent("DelayedCallbackEvent");
            delayedCheck.bind(function() {
                print("=== POSITION CHECK AFTER 1 SECOND ===");
                var newPos = script.surfaceDetection.getTransform().getWorldPosition();
                print("Surface detection position after sync: (" + newPos.x + "," + newPos.y + "," + newPos.z + ")");
                
                for (var i = 0; i < script.surfaceDetection.getChildrenCount(); i++) {
                    var child = script.surfaceDetection.getChild(i);
                    var childPos = child.getTransform().getWorldPosition();
                    print("Child " + child.name + " position after sync: (" + childPos.x + "," + childPos.y + "," + childPos.z + ")");
                }
            });
            delayedCheck.reset(1.0);
            
        } else {
            print("ERROR: script.surfaceDetection is null!");
        }
        
        // Force enable game objects too
        showGameObjects(true);
        print("Game objects forced to visible for non-host");
        
    } else {
        print("Host received own objects placed event (isHost = " + isCurrentHost + ")");
    }
    
    print("About to change state to OBJECTS_PLACED");
    setState(GameState.OBJECTS_PLACED);
    print("State changed to OBJECTS_PLACED successfully");
}
