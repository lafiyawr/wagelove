...
function showWaitingForPlayers() {
    ...
    if (syncEntity && isHost() !== undefined) {
        ...
        enableSurfaceTracking(false); // Hide surface tracking until host places it
...
function showReadyToStart() {
    ...
    if (isHost()) {
        ...
        // Make sure surface tracking stays enabled
        enableSurfaceTracking(true);
        ...
        // Make sure surface tracking stays enabled for non-host too
        enableSurfaceTracking(true);
        print("Non-host ready state: Surface tracking kept enabled");
    }
}
