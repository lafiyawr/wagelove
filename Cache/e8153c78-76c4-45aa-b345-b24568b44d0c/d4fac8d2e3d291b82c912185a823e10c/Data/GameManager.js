...
var ancestorSelectionStarted = false; // Add this global variable at the top
...
syncEntity.onEventReceived.add('ancestorFusion', function(eventData, messageInfo) {
    onReceiveAncestorFusion(eventData, messageInfo);
});
...
function isHost() {
    // In single player mode (no session), always consider as host
    if (!global.sessionController || totalPlayersCount <= 1) {
        return true;
    }
    
    // In multiplayer, check sync entity
    return syncEntity && syncEntity.doIOwnStore();
}



// Add this helper function to detect single player mode consistently:
function isSinglePlayerMode() {
    return !global.sessionController || !syncEntity || totalPlayersCount <= 1;
}
...
function startSinglePlayer() {
    ...
    // FORCE set single player count immediately
    totalPlayersCount = 1;
    print("*** FORCED totalPlayersCount to 1 for single player ***");
...
function showReadyToStart() {
    ...
    // Check if this is single player mode (use totalPlayersCount which is now correctly set)
    var isSinglePlayerMode = !global.sessionController || totalPlayersCount <= 1;
...
function startAncestorSelection() {
    ...
    if (ancestorSelectionStarted) {
        print("Ancestor selection already started - ignoring duplicate call");
        return;
    }
    
    print("Starting ancestor selection phase");
     script.voiceManager.enabled = false;
    ancestorSelectionStarted = true; // Set the flag
...
function playAncestorFusion() {
    print("Playing ancestor fusion sequence");
      print("*** Player calling fusion: " + (isHost() ? "HOST" : "NON-HOST") + " ***");
    playVO(4);
    ...
    delayedEvent.reset(14);
     ...
     delayedEnding.bind(function() {
          print("*** About to transition to game complete ***");
...
function showGameComplete() {
    ...
    print("*** Called by player: " + (isHost() ? "HOST" : "NON-HOST") + " ***");
    print("Game complete!");
}
...
function sendAncestorsRise() {
    if (ancestorsRiseTriggered) {
        print("*** Ancestors rise already triggered - ignoring ***");
        return;
    }
    ...
    if (isSinglePlayerMode) {
        ...
        // Any player can send the event now (VoiceManager handles prioritization)
        syncEntity.sendEvent('ancestorsRisemsg');
    } else {
        print("*** ERROR: syncEntity not available ***");
    }
    
    print("*** sendAncestorsRise completed ***");
}
...
function sendPlayerCompleted() {
    ...
    print("*** DEBUG: syncEntity exists: " + !!syncEntity + " ***");
    
    // FORCE correct player count for single player
    if (isSinglePlayerMode()) {
        totalPlayersCount = 1;
        print("*** FORCED: Single player count set to 1 ***");
    }
    
    print("*** DEBUG: totalPlayersCount is now: " + totalPlayersCount + " ***");
    
    if (isSinglePlayerMode()) {
        print("*** SINGLE PLAYER: Calling completion handler directly ***");
        // In single player, just call the handler directly - no network needed
        var eventData = {
            userId: "single_player",
            userName: "Player",
            timestamp: Date.now()
        };
        
        // ALSO force the count in the global variables
        globalSelectionCount = 0; // Reset first
        
        onReceivePlayerCompleted(eventData, null);
        
    } else if (syncEntity) {
        // Multiplayer mode - send network event
        var eventData = {
        ...
        print("*** DEBUG: Sending event with data: " + JSON.stringify(eventData) + " ***");
        print("Sending player completed event for: " + eventData.userName);
        syncEntity.sendEvent('playerCompletedSelections', eventData);
        print("*** DEBUG: Network event sent successfully ***");
        
    } else {
        print("*** ERROR: syncEntity is null - cannot send event ***");
    }
}
...
function sendPlayerHealed() {
    ...
    if (isSinglePlayerMode) {
        print("*** SINGLE PLAYER: Calling earth healed handler directly ***");
        // In single player, just call the handler directly - no network needed
        var eventData = {
            userId: "single_player",
            userName: "Player",
...
// Add to your sender functions:
// Replace your sendAncestorFusion function with this:
function sendAncestorFusion() {
    print("*** sendAncestorFusion called ***");
    
    // Check if this is single player mode
    var isSinglePlayerMode = !global.sessionController || totalPlayersCount <= 1;
    
    if (isSinglePlayerMode) {
        print("*** SINGLE PLAYER: Calling ancestor fusion directly ***");
        // In single player, just call the function directly
        onReceiveAncestorFusion();
    } else if (syncEntity && syncEntity.doIOwnStore()) {
        print("*** MULTIPLAYER HOST: Sending ancestor fusion network event ***");
        // In multiplayer, send the network event to all players
        syncEntity.sendEvent('ancestorFusion');
    } else {
        print("*** ERROR: Cannot send ancestor fusion - not host or syncEntity missing ***");
    }
    
    print("*** sendAncestorFusion completed ***");
}
...
function onReceivePlayerCompleted(eventData, messageInfo) {
    print("*** DEBUG: onReceivePlayerCompleted called! ***");
    
    // ALWAYS ensure correct count for single player at the start
    if (isSinglePlayerMode()) {
        totalPlayersCount = 1;
        print("*** DEFENSIVE: Ensured totalPlayersCount = 1 for single player ***");
    }
    ...
    print("*** Is single player mode: " + isSinglePlayerMode() + " ***");
    
    if (globalSelectionCount >= totalPlayersCount) {
        print("*** SUCCESS: All players completed selections! ***");
        
        if (isSinglePlayerMode()) {
            print("*** SINGLE PLAYER: Advancing directly to ready to fuse ***");
            // In single player, call the next function directly
            var eventData = {
                userId: "single_player", 
                userName: "Player",
                timestamp: Date.now()
            };
            onReceiveReadyToFuse(eventData, null);
        } else {
            print("*** MULTIPLAYER: Sending ready to fuse event ***");
            var delayedEvent = script.createEvent("DelayedCallbackEvent");
            delayedEvent.bind(function() {
                sendReadyToFuse();
            });
        ...
        print("*** Waiting for more players to complete selections ***");
    }
}
...
function onReceivePlayerHealed(eventData, messageInfo) {
    print("*** onReceivePlayerHealed called ***");
    
    // Ensure we have correct count for single player
    if (!global.sessionController && totalPlayersCount === 0) {
        totalPlayersCount = 1;
        print("*** CORRECTED: Set totalPlayersCount to 1 for single player ***");
    }
    ...
    print("*** DEBUG: Player receiving this: " + (isHost() ? "HOST" : "NON-HOST") + " ***");
    
    if (globalEarthHealedCount >= totalPlayersCount) {
        print("All players healed the earth!");
        print("*** DEBUG: About to check if should send ancestor fusion ***");
        
        var isSinglePlayerMode = !global.sessionController || totalPlayersCount <= 1;
        print("*** DEBUG: isSinglePlayerMode: " + isSinglePlayerMode + ", isHost: " + (syncEntity && syncEntity.doIOwnStore()) + " ***");
        if ((syncEntity && syncEntity.doIOwnStore()) || isSinglePlayerMode) {
            print("*** DEBUG: HOST/SINGLE_PLAYER sending ancestor fusion event ***");
            currentState = GameState.ANCESTOR_FUSION;
            sendAncestorFusion();
        } else {
            print("*** DEBUG: NON-HOST not sending event, waiting for host ***");
        }
    } else {
        print("*** Waiting for more players to heal the earth ***");
    }
}

function onReceiveAncestorFusion(eventData, messageInfo) {
    print("*** RECEIVED ANCESTOR FUSION EVENT ***");
    print("*** Player receiving: " + (isHost() ? "HOST" : "NON-HOST") + " ***");
    currentState = GameState.ANCESTOR_FUSION;
    playAncestorFusion();
}
...
function updatePlayerCount() {
    if (global.sessionController) {
        ...
        // No session controller means single player
        totalPlayersCount = 1;
        print("No session controller - setting single player count to 1");
    }
    ...
    if (currentState == GameState.WAITING_FOR_PLAYERS && 
        ...
        enableReadyButton(false);
    } else if (!isHost() && totalPlayersCount > 1) { // Only disable for non-host in multiplayer
