...
function showObjectsPlaced() {
    ...
    if (isHost()) {
        ...
        print("NON-HOST: Assets should now be visible via SyncTransform components");
        
        setUIVisibility("hostWaitingMessage", true);
        updateHostWaitingMessage();
        enableReadyButton(false);
        
        // Enable surface detection object for SyncTransform to work, but keep scripts disabled
        if (script.surfaceDetection) {
            script.surfaceDetection.enabled = true;
            print("Surface detection object enabled for SyncTransform (scripts stay disabled)");
        }
    }
}
...
function onReceiveObjectsPlaced(eventData, messageInfo) {
    ...
    if (!isHost()) {
        print("NON-HOST: Received surface placement event, enabling assets");
        
        // Enable the surface detection object so SyncTransform components work
        if (script.surfaceDetection) {
            script.surfaceDetection.enabled = true;
            print("Surface detection object enabled for SyncTransform");
            
            // Enable all children recursively
            enableAllChildren(script.surfaceDetection);
        }
        
        // Assets should now be visible via SyncTransform components
        print("NON-HOST: Assets should now be synced and visible");
...
// Helper function to enable all children recursively
function enableAllChildren(parentObject) {
    var childCount = parentObject.getChildrenCount();
    for (var i = 0; i < childCount; i++) {
        var child = parentObject.getChild(i);
        child.enabled = true;
        print("Enabled child: " + child.name);
        
        // Recursively enable grandchildren
