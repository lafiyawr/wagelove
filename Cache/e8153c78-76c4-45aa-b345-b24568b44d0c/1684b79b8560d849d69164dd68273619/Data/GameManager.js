...
function enableSurfaceTracking(enable) {
    if (script.surfaceDetection) {
        ...
        for (var i = 0; i < scriptComponents.length; i++) {
            // Check if this is the SurfaceDetection script - add null checks
            if (scriptComponents[i] && scriptComponents[i].script && scriptComponents[i].script.name) {
                var scriptName = scriptComponents[i].script.name;
                if (scriptName === "SurfaceDetection" || scriptName.indexOf("SurfaceDetection") !== -1) {
        ...
        if (surfaceDetectionScript) {
            ...
            // Fallback: try to get any script component and enable/disable it
