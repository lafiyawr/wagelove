...
// Runtime modifiable variables - initialized from script inputs
var runtimeSpawnRate;
var runtimeGravity;
var runtimeWind;
var runtimeDrag;

function initialize() {
    // Copy values from script inputs to runtime variables
    runtimeSpawnRate = script.spawnRate;
    runtimeGravity = new vec3(script.gravity.x, script.gravity.y, script.gravity.z);
    runtimeWind = new vec3(script.wind.x, script.wind.y, script.wind.z);
    runtimeDrag = script.drag;
...
function handleEmission() {
    ...
    if (script.burstCount > 0) {
        // Burst mode
        if (currentTime - lastSpawnTime > 1.0 / runtimeSpawnRate) {
            spawnBurst(script.burstCount);
            lastSpawnTime = currentTime;
        }
    } else {
        // Continuous mode
        var spawnInterval = 1.0 / runtimeSpawnRate;
...
function destroyParticle(particleData) {
    ...
    // Simple immediate cleanup - no fancy animations to avoid compatibility issues
...
function calculateSpawnPosition() {
    ...
    switch (script.emissionShape) {
        ...
        case "Cone":
            ...
            var perpendicular = new vec3(1, 0, 0); // Simple perpendicular vector
            if (Math.abs(coneDir.x) > 0.9) {
                perpendicular = new vec3(0, 1, 0);
            }
            
            var rotatedDir = coneDir.add(perpendicular.scale(Math.sin(angle) * randomRadius));
            return basePos.add(rotatedDir.normalize().scale(randomRadius));
...
function calculateSpawnVelocity() {
    // Base velocity
    var velocity = new vec3(script.initialVelocity.x, script.initialVelocity.y, script.initialVelocity.z);
...
function updateParticlePhysics(particle, age) {
    // Apply forces using runtime values
    particle.velocity = particle.velocity.add(runtimeGravity.scale(getDeltaTime()));
    particle.velocity = particle.velocity.add(runtimeWind.scale(getDeltaTime()));
    
    // Apply drag
    particle.velocity = particle.velocity.scale(runtimeDrag);
...
function setEmissionRate(rate) {
    runtimeSpawnRate = rate;
    print("Emission rate set to: " + rate);
}

function setGravity(gravityVec) {
    runtimeGravity = new vec3(gravityVec.x, gravityVec.y, gravityVec.z);
    print("Gravity set to: " + gravityVec.toString());
}

function setWind(windVec) {
    runtimeWind = new vec3(windVec.x, windVec.y, windVec.z);
    print("Wind set to: " + windVec.toString());
}
...
script.createEvent('UpdateEvent').bind(updateSystem);